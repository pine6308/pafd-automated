...
class ReminderManager {
    ...
    setupMidnightRefresh() {
        ...
        console.log(`Midnight refresh scheduled in ${Math.round(msUntilMidnight / 1000 / 60)} minutes`);
    }
    ...
    // 获取小红花存储键
    getFlowerKey(date, type) {
        return `${FLOWERS_KEY_PREFIX}:${date}:${type}`;
    }
    // 获取今日某类型的小红花数量
    getTodayFlowers(type) {
    ...
    getAllTodayFlowers() {
        ...
        ['standup', 'water', 'kegel', 'neck'].forEach((type) => {
            const key = this.getFlowerKey(today, type);
            const data = this.store.get(key);
